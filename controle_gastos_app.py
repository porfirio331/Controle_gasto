...
salario = st.number_input("Digite seu salário (R$)", min_value=0.0, format="%.2f", value=st.session_state.salario)
...
if not st.session_state.dados.empty:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        pd.DataFrame([{'Salário': st.session_state.salario, 'Total Gasto': total_gasto, 'Saldo Restante': saldo_restante}]).to_excel(writer, index=False, sheet_name='Resumo')
    output.seek(0)
    col2.download_button(
        label="Baixar como Excel",
        data=output,
        file_name="controle_gastos_com_salario.xlsx",
