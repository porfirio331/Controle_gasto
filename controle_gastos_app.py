...
# Define o local para formatação brasileira
...
salario = st.text_input("Digite seu salário (R$)", value=f"{st.session_state.salario:,.2f}".replace('.', ',').replace(',', '#').replace('.', ',').replace('#', '.'))
...
with st.form("formulario_gastos"):
    ...
    valor_str = st.text_input("Valor (R$)", placeholder="Ex: 12.500,00")
...
col1, col2 = st.columns(2)
col1.metric("Total Gasto", locale.currency(total_gasto, grouping=True, symbol=True))
col2.metric("Saldo Restante", locale.currency(saldo_restante, grouping=True, symbol=True))
...
if not st.session_state.dados.empty:
    ...
    col2.download_button(
        ...
        data=output,
        file_name="controle_gastos_formatado.xlsx",
